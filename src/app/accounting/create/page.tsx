...
export default function CreateTransactionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('https://erp-backend-production-3dd4.up.railway.app/api/accounting/transactions', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        throw new Error(`Failed to create transaction. Status: ${res.status}`);
      }
